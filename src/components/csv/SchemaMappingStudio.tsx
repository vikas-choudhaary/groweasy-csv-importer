...
import { Database, Robot, FileCsv, CheckCircle, Question, ArrowsLeftRight, Lightning, MagicWand, FileArrowUp, Check, Play, Warning, Trash, CalendarBlank, Hash, ArrowLeft, ArrowRight, UserList, MagnifyingGlass, Funnel, PencilSimple, X, Link, LinkBreak } from "@phosphor-icons/react";
...
export function SchemaMappingStudio({ headers, records, onContinue, onBack }: SchemaMappingStudioProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const schemaRes = await fetch(`${baseUrl}/api/schema/metadata`);
